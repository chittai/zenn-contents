---
title: "Kiro CLI 2.0.0 がリリースされたのでドキュメントを読んでKiro CLIの機能を整理してみた"
emoji: "🗺️"
type: "tech"
topics: ["kiro", "aws", "ai"]
published: true
...
v2.0.0 で追加されたばかりの機能です。API key 認証で Kiro CLI を **非対話モードで実行できる** ようになりました。要するに、CI/CD パイプラインや cron で Kiro エージェントを回せるようになったということです。
...
ツール承認も事前に指定できて、最小権限でスクリプト実行するなら `--trust-tools` を使います。
...
- **Pro / Pro+ / Power プラン限定**（Free プランでは使えない）
- 対話形式のコマンド（`/model`、`/agent` ピッカー等）は使えない
- API key は CI/CD のシークレットとして管理する必要あり

ドキュメントのサンプルにあるようにGitHub Actionsでレビューに使ったりするのが一番イメージしやすいですね！
...
Kiro CLI について整理してみて、以前よりも機能が充実してきたという感想です。Q Developer CLI 時代から使ってきた身としては、リブランディング後も `aws` ツールが健在なのは安心しました。ちょうど記事を書いている最中に v2.0.0 がリリースされ、Windows ネイティブサポートや Headless mode が追加されたので、今後は CI/CD への組み込みも含めてさらに活用の幅が広がりそうです。また新しい機能が CLI に来たら改めて評価したいです。
